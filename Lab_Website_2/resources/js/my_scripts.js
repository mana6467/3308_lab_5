...
function loadStatsPage(){
	...
	{//Outer for loop iterates over each row
		awayName = table.rows[row_counter].cells[1].innerHTML;
		cuScore = table.rows[row_counter].cells[2].innerHTML;
		awayScore = table.rows[row_counter].cells[3].innerHTML;
			if (parseInt(cuScore) < parseInt(awayScore)){
					table.rows[row_counter].cells[4].innerHTML = awayName;
					losses++;
			}
			else {
...
function loadPlayersPage(){
	...
	for(i = 0; i < players.length; i++){
		actualCode = actualCode + "<a href=\"#\" class=\"dropdown-item\" onclick=\"switchPlayers("+i+")\">"+players[i].name+"</a>";
	}
	document.getElementById("player_selector").innerHTML = actualCode;
}
